...
const char MQTTUSER[]    =  "username";                  // имя пользователя (логин)  (смотрим в аккаунте MQTT)
const char MQTTPASSWORD[] = "securepassword";                // пароль                    (смотрим в аккаунте MQTT)
const char SERVERNAME_PORT[] = "\"mqtt.mosquitto.com\", \"1883\"";  // адрес и порт (смотрим в аккаунте MQTT)
...
//------------------- распиновка ног ардуино (плата весий 8.8-8.93)--------------------------------------------------------
...
#define DopOn_pin           4  // сюда доп канал от сигналки на включение вебасто
#define DopOff_pin          5  // сюда доп канал от сигналки на выключение вебасто
#define Ohrana_pin          6  // Сюда состояние охраны сигналки
#define Trevoga_pin         7  // Сюда состояние тревоги
#define IGN_pin             8  // Сюда состояние зажигания
#define Sost_pin            9 // Сюда состояние вебасто (+12В когда работает)
#define ResetGSM           12  // пин ресета GSM подключен к реле, разрывающее питание модуля. 
#define PUMPE              13  // пин управления помпой
#define Eng_pin            14  // (А0) Сюда состояние работы ДВС
...
#define OutWebasto_GndImp  19  // пин (A5) это импульсный минусовой выход вкл/выкл вебасто (к впайке к кнопке таймера). 
#define SLDATA_TX          20  // этот пин цеплять на цифровую шину старлайн SL-DATA (на RX - левый пин если смотреть на разъем сиги. Центр там GND . Правый это Tx)
...
enum BuTTons {StartButton, DopOnButton, DopOffButton, EngineButton, IGNButton, OhranaButton, TrevogaButton, SostButton};

//-----------------------------для шины SL_DATA сигнализаций старлайн-----------------------------------------------------

#define BIT_DURATION  190  //время одного бита в микросекундах для шины SL_DATA
const uint16_t SL_START_ENG  = 8670;
const uint16_t SL_STOP_ENG   = 8415;
const uint16_t SL_OHRANA_ON  = 4590;
const uint16_t SL_OHRANA_OFF = 4335;
...
const char MQTT_ON[]        PROGMEM = "MQTT-ON";
const char MQTT_OFF[]       PROGMEM = "MQTT-OFF";
...
const char CTRL_OHR_TOP[]     PROGMEM = "c/ohr";
...
const char STAT_WPUMP_TOP[]   PROGMEM = "s/pom";
const char STAT_PLUG_TOP[]    PROGMEM = "s/plg";
const char STAT_FAN_TOP[]     PROGMEM = "s/aif";
const char STAT_FPUMP_TOP[]   PROGMEM = "s/fue";
...
dallasSens DS18B20[size_arrayTemp] = 
 {
    { {0x28, 0xFF, 0xB2, 0xB5, 0xC1, 0x17, 0x05, 0xD1}, -100, "Vyhlop" },
    { {0x28, 0xFF, 0xD3, 0xE2, 0xC1, 0x17, 0x04, 0x0D}, -100, "Engine" },
    { {0x28, 0xFF, 0xF8, 0xBC, 0xC1, 0x17, 0x04, 0x48}, -100, "Ulica"  },
    { {0x28, 0xFF, 0x3F, 0xB7, 0xC1, 0x17, 0x05, 0xF1}, -100, "Salon"  }
 };
...
//enum BuTTons {StartButton,    DopOnButton, DopOffButton, EngineButton, IGNButton, OhranaButton, TrevogaButton, SostButton};
test.button    (StartButtonpin, DopOn_pin,   DopOff_pin,   Eng_pin,      IGN_pin,   Ohrana_pin,   Trevoga_pin, Sost_pin);
 
pinMode (OutWebasto_12V,     OUTPUT);  digitalWrite (OutWebasto_12V,      LOW);
pinMode (SLDATA_TX ,         OUTPUT);  digitalWrite (SLDATA_TX ,         HIGH);
...
pinMode (OutWebasto_GndImp,  OUTPUT);  digitalWrite (OutWebasto_GndImp,   LOW);
...
pinMode (ResetGSM,           OUTPUT);  digitalWrite (ResetGSM,        !RelayON);  // реле ресет на данный момент делаем "неактивно"
  ...
  SIM800.begin(9600);           // сериал соединение для gsm модуля
...
for (byte i = 0; i<size_arrayTemp; i++) 
    {
    for (byte k=0; k<sizeof(DS18B20[i].DSaddress); k++) DS18B20[i].DSaddress[k] = EEPROM.read(DallasAddr_cell+i*10+k);
    }
