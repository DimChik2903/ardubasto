...
if (ProtocolSTATUS==STATUSBUS && (currmillis - last_Flame>30000) && ProtocolSTATUS==STATUSBUS) {startWebasto_OK=0; last_Flame = currmillis;}  // делаем статус "нет пламени" через 30 сек, если не получаем сообщения от котла

if (Initreset && currmillis - prevInitreset>10000) {Initreset = 0; w_bus_init = 0; noData = 1 ;}  // сброс инита, если прошло более 18 сек после отправки последнего сообщения
...
#ifdef  Signalka
    void SL_sendword (uint16_t SL_inbyte)
{
  ...
  for (int e =0; e<16; e++)
  {
  digitalWrite (SLDATA_TX ,  !bitRead(SL_inbyte, e)); //send data bits
  delayMicroseconds (BIT_DURATION); 
  }
}

void SL_sendcommand (uint16_t SL_comm)
{
