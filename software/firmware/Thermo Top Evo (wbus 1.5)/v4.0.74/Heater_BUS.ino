...
noData = 0;
   ...
   if (buf[2]==0xD2 )  {DTCareCleared();}  // если получили ответ на команду стирания ошибок                 
...
if (w_bus_init==10 && HeaterBUSTYPE == WBUS)
{
    if (needAction>0 && needAction < NEED_DTCCLEAR) 
   {
    ...
    if (needAction==NEED_DTCCLEAR) { sendMessage (DTC_ERASE_wbus15, sizeof(DTC_ERASE_wbus15)); w_bus_init = 20;  }
}

if ((NeedTimer && currmillis - prevNeedTimer>11000) || w_bus_init == 16)  {NeedTimer = 0; 
