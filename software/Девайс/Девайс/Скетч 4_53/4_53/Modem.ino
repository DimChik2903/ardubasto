...
void Check_responseModem ()
{
  ...
  if (failresets <= 4)   //если MQTT активен
  {
    ...
    static bool last_ohrana = 0;
    static bool last_treva = 0;
    if (last_treva != trevoga || last_ohrana!= ohrana || last_engine != engine || last_MQTTwebasto != webasto || last_startwebasto_OK != startWebasto_OK && (w_bus_init < 1 || w_bus_init > 6))
    {
      MQTTsendDatastream(); last_treva = trevoga; last_engine = engine; last_MQTTwebasto = webasto; last_startwebasto_OK = startWebasto_OK; last_ohrana = ohrana;
    }
    if ((webasto || engine) && currmillis - prev_refreshMQTT > Refresh_interval_heaterON) {
      MQTTsendDatastream();
...
void NastroykaGSM () {
  ...
  else if (currmillis - prevGSMnastr > delayATcommand)
  {
...
#ifdef DEBUG_MODEM
    else if (gsmstatus == echoOFF)          SIM800.println (F("ATE1")); //SIM800.println (F("ATE0"));
#else
    ...
    else if (gsmstatus == setSubPack)       SUBsend("c/#");
...
void SMSDallasAddr()
{
  ...
  {
    SIM800.print(DS18B20[i].TempName); SIM800.println();
    for (byte k = 0; k < 8; k++) {
      if (DS18B20[i].DSaddress[k] <= 0x0F)SIM800.print(F("0"));
      SIM800.print(DS18B20[i].DSaddress[k], HEX);
      SIM800.print(F(" "));
    }
    SIM800.println();
  }
...
{
  for (byte i = 0; i < 2; i++) {
    byte number_is_saved = EEPROM.read(((i+1)*20)+1);
    if (number_is_saved==48 || number_is_saved==0xFF) break;
    startSMS(i + 1);
...
/*    SIM800.println(F("Temperatures:"));
    if (ProtocolSTATUS != ANALOG)
    {
      ...
      if ((needAction > 0 && !noData) || needAction == 0) {
        SIM800.print (HeaterC);
        grad ();
      }
 ...
 //   for (byte i = 0; i < size_arrayTemp; i++)
 //   {
 //     if (i == VyhlopC && ProtocolSTATUS != ANALOG) {}
 //     else {
 //       SIM800.print (DS18B20[i].TempName);
 //       SIM800.print (DS18B20[i].Temper);
 //       grad ();
 //     }
 //   }
*/
...
void on_off (const bool &stat) {
  if (stat) SIM800.println (F("ON"));
...
{
  startSMS(telef);
  SIM800.print(F("MQTT     ")); on_off (mqtt);
  SIM800.print(F("Heater:  "));
  ...
  else if (ProtocolSTART == STARTBUS)  {
    SIM800.print(F("BUS"));
  ...
  if (ProtocolSTART != IMPULSE) {
    SIM800.print(F("Webasto Time: "));
    SIM800.print (TimeWebasto);
    SIM800.println(F("min"));
  }
  SIM800.print(F("Modem Resets: ")); SIM800.println (ResetNumber);

  if (ProtocolSTATUS == ANALOG) {
    SIM800.print(F("DeltaT:  "));
    SIM800.print(delta);
    grad ();
  }
...
void Is_not_configured() {
  SIM800.println(F("Is not configured"));
  EndSMS();
}
