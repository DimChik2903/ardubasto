    ...
    if (currSymb[0] == '\r' || currSymb[0] == '&' || stringEnd == 1)
       {
         ...
         if (isStringMessage>0&&isStringMessage<3) //если текущая строка - SMS-сообщение, отреагируем на него соответствующим образом
                {
     if (strcmp(currStr, PGM_Text(_ZAPROS))==0 || strcmp(currStr, "41")==0)       {if (Initreset) SMSzapros(isStringMessage);   else {KTOzapros = isStringMessage; Queue(NEED_SMSZAPROS);}}       // Передача параметров по СМС
...
else if (strcmp(currStr, PGM_Text(_MQTT_ON))==0)      {startSMS(isStringMessage);  mqtt = 1; SIM800.println (PGM_Text(_MQTT_ON)); EndSMS ();}
else if (strcmp(currStr, PGM_Text(_MQTT_OFF))==0)     {startSMS(isStringMessage);  mqtt = 0; failresets = 5; SIM800.println (PGM_Text(_MQTT_OFF)); EndSMS ();}

else if (strcmp(currStr, PGM_Text(_HEATER_ON))==0 || strcmp(currStr, "221")==0)  {  startSMS(isStringMessage); SIM800.println(F("Webasto ")); 

  
           if (!webasto)  {StartWebasto ();smszapusk=1;  KTOreport = isStringMessage;}
           else SIM800.println(F("uzhe ")); SIM800.println (F("vkluchena")); EndSMS();}

                                                            
else if (strcmp(currStr, PGM_Text(_HEATER_OFF))==0 || strcmp(currStr, "220")==0)   {startSMS(isStringMessage); SIM800.println(F("Webasto "));  
...
else if (strcmp(currStr, PGM_Text(_ENGINE_ON))==0 || strcmp(currStr, "21")==0)  {startSMS(isStringMessage); SIM800.println(F("Dvigatel "));  
           if (!engine)  { Start_Engine(); reportEngine = true; prevReportEngine = currmillis; KTOreport = isStringMessage;} 
           else {SIM800.println(F("uzhe "));}  SIM800.println(F("start")); EndSMS();}
                                                                                 
else if (strcmp(currStr, PGM_Text(_ENGINE_OFF))==0 || strcmp(currStr, "20")==0)   {startSMS(isStringMessage);  SIM800.println(F("Dvigatel "));  
           if (engine){ SL_sendcommand(SL_STOP_ENG); reportEngine = false;} // если получили команду на выключение ДВС и он в настоящий момент работает - выключаем
           else SIM800.println(F("uzhe ")); SIM800.println(F("ostanovlen")); EndSMS();}   
else if (strcmp(currStr, "11")==0)     {startSMS(isStringMessage); SL_sendcommand (SL_OHRANA_ON) ;  SIM800.println (F("Ohrana ON")); EndSMS ();}             
else if (strcmp(currStr, "10")==0)     {startSMS(isStringMessage); SL_sendcommand (SL_OHRANA_OFF) ; SIM800.println (F("Ohrana OFF")); EndSMS ();}  
#endif 
                                            ...
                                            startSMS(isStringMessage); SIM800.print (PGM_Text(_HEATERBUSTYPE)); SIM800.print (PGM_Text(_SPACE));
                                                    ...
                                                    if (savadr>=0 && savadr<=3) {SIM800.print(DS18B20[savadr].TempName); SIM800.println (F("saved OK!")); 
                                                    SMSDallasAddr();
                                                                                } 
  ...
  if (strstr(currStr, PGM_Text(_CMT))>0) cmT = 1;
     ...
     if (strcmp (TelNumber[1],TelNumber[0])!=0 && !SaveNumber2)
      ...
      SIM800.print(F("Tel#1: ")); SIM800.println(TelNumber[1]); SIM800.print(F("Tel#2: "));  SIM800.println (TelNumber[2]); EndSMS(); SaveNumber2 = 0; } 
                  ...
                  else if (last_signalLevelstatus==0 && signalLevelstatus==3) {last_signalLevelstatus  = 3; prevModemReboot = currmillis; if (mqtt)smsModemReboot_timer = 1;   }// если появился сигнал сети
                       }
...
#ifdef Signalka
      if (strstr(currStr, PGM_Text(_CTRL_DVS_TOP))    > 0)  // если получили команду старт/стоп двс
      { 
        bool comanda = (bool)atoi(strstr (currStr,PGM_Text(_CTRL_DVS_TOP)) + strlen(PGM_Text(_CTRL_DVS_TOP)));
        if (comanda) {Start_Engine(); if (!engine)  { reportEngine = 0;}} 
        else {SL_sendcommand(SL_STOP_ENG);}
      }

      if (strstr(currStr, PGM_Text(_CTRL_OHR_TOP))    > 0)  // если получили команду изменить состояние охраны сигнализации
      { 
        bool comanda = (bool)atoi(strstr (currStr, PGM_Text(_CTRL_OHR_TOP)) + strlen(PGM_Text(_CTRL_OHR_TOP)));
        if (comanda) {SL_sendcommand (SL_OHRANA_ON) ;} 
        else         {SL_sendcommand (SL_OHRANA_OFF);}
      }
#endif
