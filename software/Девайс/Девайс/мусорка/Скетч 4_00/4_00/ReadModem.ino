    ...
    if (currSymb[0] == '\r' || currSymb[0] == '&' || stringEnd == 1)
       {
         ...
         if (isStringMessage>0&&isStringMessage<3) //если текущая строка - SMS-сообщение, отреагируем на него соответствующим образом
                {
     if (strcmp(currStr, PGM_Text(_ZAPROS))==0)       {Queue(NEED_SMSZAPROS);}             // Передача параметров по СМС
else if (strcmp(currStr, PGM_Text(_SERVICE_INFO))==0) {Queue(NEED_SERVICEINFO);}           // Передача сервисной информации  по СМС
else if (strcmp(currStr, PGM_Text(_ERASE_DTC))==0)    {Queue(NEED_DTCCLEAR);}              // Запрос на стриание ошибок
else if (strcmp(currStr, PGM_Text(_ZAPROSTEL))==0)    {SMSzaprosTEL();}                    // Передача номеров телефонов пользователей по СМС
else if (strcmp(currStr, PGM_Text(_GSM_RESETS))==0)   {ResetNumber=0; EEPROM.write (ResetNumber_cell, ResetNumber); Queue(NEED_SERVICEINFO);}     //сброс счетчика ресетов GSM модуля
else if (strcmp(currStr, PGM_Text(_VERSION))==0)      {startSMS(isStringMessage);  SIM800.println (ver); EndSMS ();}  //запрос версии ПО
else if (strcmp(currStr, PGM_Text(_MQTT_RESET))==0)   {prevModemReboot = currmillis; smsModemReboot_timer = 1;}       // рестарт модема и MQTT 
...
HEATER_PRESENCE[1] = StartByte;
            ...
            isStringMessage = 0;
              ...
              startSMS(1); SIM800.println(F("Tel Number#1 is saving in memory"));  SIM800.print("Tel#1: ");  SIM800.println (TelNumber[1]); EndSMS();
                                             ...
                                             else if (strcmp(currStr, PGM_Text(_ZAPROS))==0)      { SMSzapros();}   
                                             else if (strcmp(currStr, PGM_Text(_ZAPROSTEL))==0)   { SMSzaprosTEL();}               // Передача номеров телефонов пользователей по СМС            
               isStringMessage = 0;
...
//-------------------------топики MQTT, принимаемые от брокера
      ...
      if (strstr(currStr, CTRL_HEAT_TOP) > 0) 
      {
        bool comanda = (bool)atoi (strstr (currStr, CTRL_HEAT_TOP) + strlen(CTRL_HEAT_TOP));
        if (comanda) {StartWebasto(); report = false;} 
        else {StopWebasto();}
      }
      if (strstr(currStr, CTRL_TIME_TOP) > 0) {
        TimeWebasto = atoi (strstr (currStr,CTRL_TIME_TOP) + strlen(CTRL_TIME_TOP));
        EEPROM.write(TimeWebasto_cell,TimeWebasto);
        PUBsend(STAT_TIME_TOP,  Var_to_text(TimeWebasto,0), 0);
        remTime(0) ;
      }
        if (strstr(currStr, CTRL_TIMEENG_TOP) > 0) { 
        TimeEngine = atoi (strstr (currStr,CTRL_TIMEENG_TOP) + strlen(CTRL_TIMEENG_TOP));
        PUBsend(STAT_ENGTIME_TOP,  PGM_Text(_OK), 0);
        }
      
      if (strstr(currStr, CTRL_REFRESH_TOP) > 0) {if (!webasto) {Queue(NEED_MQTTZAPROS); delay (100);}   MQTTsendDatastream();}
      if (strstr(currStr, CTRL_RESRES_TOP)  > 0) {ResetNumber=0; EEPROM.write (ResetNumber_cell, ResetNumber); PUBsend(STAT_RESETS_TOP,Var_to_text(ResetNumber,0),0);}
      
      if (strstr(currStr, CTRL_LOCK_TOP)    > 0 && CentrLock)  // если получили команду открыть/закрыть замки
      { 
        bool comanda = (bool)atoi(strstr (currStr, CTRL_LOCK_TOP) + strlen(CTRL_LOCK_TOP));
...
#ifdef Signalka
      if (strstr(currStr, CTRL_DVS_TOP)    > 0 && !CentrLock)  // если получили команду старт/стоп двс
      { 
        bool comanda = (bool)atoi(strstr (currStr,CTRL_DVS_TOP) + strlen(CTRL_DVS_TOP));
