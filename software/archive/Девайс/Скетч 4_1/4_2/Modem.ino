...
SIM800.println (F("Dallas Addresses:"));
  for (byte i = 0; i<size_arrayTemp; i++) 
  {
       ...
       for (byte k = 0; k<8; k++) {if (DS18B20[i].DSaddress[k]<=0x0F)SIM800.print(F("0")); SIM800.print(DS18B20[i].DSaddress[k],HEX); SIM800.print(F(" "));}
       SIM800.println(); 
  }
...
void SMSzapros(const byte &telef)
{
 ...
 // ниже распечатаем все температуры   
    for (byte i = 0; i<size_arrayTemp; i++) 
    {
      ...
      else {SIM800.print (DS18B20[i].TempName); SIM800.print (DS18B20[i].Temper); grad ();}
    }
