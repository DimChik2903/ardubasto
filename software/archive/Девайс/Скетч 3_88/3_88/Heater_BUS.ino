...
else if ( timerInitflag && (millis() - timerInit>24) && w_bus_init==NEED) {timerInit = millis(); digitalWrite(TX, 1); w_bus_init=2; }
...
if ( checksum == buf[message_size+n-1]) {
   ...
   if (buf[2]==0xD1 && buf[3]==0x0A)  {w_bus_init = 10; if (WBUS_VER != buf[4]) WBUS_VER = buf[4], EEPROM.write(WBUS_VER_cell, WBUS_VER);}  //сообщение старта коммуникации 
   if ((buf[2]^0x80)==HEATER_START[0] && buf[3]==HEATER_START[1])  {StartMessageRepeat =4;} // ответ на запуск котла 
   if (buf[2]==0x90)  {StopMessageRepeat=4;} // ответ на стоп котла    
   if (buf[2]==0xD6 && buf[3]==0x03)  {startSMS (KTOzapros); SIM800.println (F("DTCs are erased!")); EndSMS(); needAction=0; w_bus_init = 9; NeedTimer = 0;  }  // ответ на запрос стирания ошибок                 
   if (buf[2]==0xD6 && buf[3]==0x01)         // ответ на запрос DTC (сообщение с кодами ошибок)
         {
...
if (needAction>0 && needAction < NEED_DTCCLEAR && w_bus_init==10) 
   {
...
if ((NeedTimer && currmillis - prevNeedTimer>11000) || w_bus_init == 14)  {NeedTimer = 0; 
        ...
        {
          cipsend_begin (); 
          PUBsend (tHEAT_TOP,   noData ? FAIL : String(HeaterC),1);
          PUBsend (R_NAPR_TOP,  noData ? FAIL : String(Vpit), 1); 
          PUBsend (R_POWER_TOP ,noData ? FAIL : String(PowerHeater), 1); 
