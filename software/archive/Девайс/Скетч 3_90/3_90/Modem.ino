void Check_responseModem ()
{
 ...
 if (failresets <= 4)   //если MQTT активен 
    ...
    if (last_MQTTwebasto != webasto || last_startwebasto_OK != startWebasto_OK && w_bus_init<1 && w_bus_init>6){MQTTsendDatastream(); last_MQTTwebasto = webasto; last_startwebasto_OK = startWebasto_OK;}
...
void NastroykaGSM () {
  ...
  else if (currmillis - prevGSMnastr > delayATcommand)
  {
    ...
    else if (gsmstatus == setSubPack)       SUBsend("d/#");
