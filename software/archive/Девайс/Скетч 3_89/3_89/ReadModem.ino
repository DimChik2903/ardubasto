...
else if (!currStr.compareTo(F("MQTT-reset")))   {prevModemReboot = currmillis; smsModemReboot_timer = 1;}       // рестарт модема и MQTT 
...
// если получили команду на включение ДВС и он в настоящий момент выключен - включаем
else if (!currStr.compareTo(F("Engine-ON")))  {startSMS(isStringMessage); if (!CentrLock) {SIM800.println(F("Dvigatel "));  
           ...
           else SIM800.println(F("uzhe ")); SIM800.println(F("start")); } else SIM800.println(F("No set")); EndSMS();}
                                                                                 
else if (!currStr.compareTo(F("Engine-OFF")))   {startSMS(isStringMessage); if (!CentrLock) { SIM800.println(F("Dvigatel "));  
           ...
           else SIM800.println(F("uzhe ")); SIM800.println(F("ostanovlen"));} else SIM800.println(F("No set")); EndSMS();}   
            ...
            isStringMessage = 0;
              ...
              startSMS(1); SIM800.println(F("Tel Number#1 is saving in memory"));  SIM800.print("Tel#1: ");  SIM800.println (TelNumber[1]); EndSMS();
               ...
               isStringMessage = 0;
                ...
                else if (currStr.startsWith(F("+CSQ:"))) //если текущая строка начинается с "+CSQ",то значит был запрос на уровень сигнала GSM
                  {
                    if (settingGSM) {gsmstatus++; fails = 0; delayATcommand = 400;}
                  signalLevel=currStr.substring(5).toInt(); 
                  if ((signalLevel<2 || signalLevel ==99) && signalLevelstatus!=0) signalLevelstatus --; 
                  else if (signalLevel>=4 && signalLevel !=99) {signalLevelstatus ++; if  (signalLevelstatus>=3)signalLevelstatus = 3;}
                  
                  if      (last_signalLevelstatus==3 && signalLevelstatus==0) {               last_signalLevelstatus  = 0; }// если пропал сигнал сети 
                  else if (last_signalLevelstatus==0 && signalLevelstatus==3) {MQTTrestart(); last_signalLevelstatus  = 3; }// если появился сигнал сети
                  }
                                                }
...
if (currStr.indexOf(F("CLOSED")) > -1 || currStr.indexOf(F("ERROR")) > -1)  {
      ...
      if (currStr.indexOf(F("CONNECT")) > -1) if (gsmstatus == setBrokerconnect) {
        ...
        if ((currStr.indexOf(F("SMS Ready"))>-1) || (currStr.indexOf(F("READY")) > -1)) if (gsmstatus == WaitGSM) {
...
//-------------------------топики MQTT, принимаемые от брокера
      ...
      if (currStr.indexOf(D_HEAT_TOP) > -1) 
      {
        bool comanda = currStr.substring(currStr.indexOf(D_HEAT_TOP) + strlen(D_HEAT_TOP)).toInt();
        if (comanda) {StartWebasto(); report = false;} 
        else {StopWebasto();}
      }
      if (currStr.indexOf(D_TIME_TOP) > -1) {
        TimeWebasto = currStr.substring(currStr.indexOf(D_TIME_TOP) + strlen(D_TIME_TOP)).toInt();
        EEPROM.write(TimeWebasto_cell,TimeWebasto);
        PUBsend(R_TIME_TOP,  String(TimeWebasto), 0);
        remTime(0) ;
      }
      if (currStr.indexOf(D_REFRESH_TOP) > -1) {if (!webasto) {Queue(NEED_MQTTZAPROS); delay (100);}   MQTTsendDatastream();}
      if (currStr.indexOf(D_RESRES_TOP)  > -1) {ResetNumber=0; EEPROM.write (ResetNumber_cell, ResetNumber); PUBsend(R_RESETS_TOP,String(ResetNumber),0);}
      
      if ((currStr.indexOf(D_LOCK_TOP)    > -1) && CentrLock)  // если получили команду открыть/закрыть замки
      { 
        bool comanda = currStr.substring(currStr.indexOf(D_LOCK_TOP) + strlen(D_LOCK_TOP)).toInt();
