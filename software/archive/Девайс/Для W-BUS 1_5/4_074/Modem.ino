...
{       
        ...
        if (_address_== EngineC) SIM800.print(F("Engine:  "));
...
void SMSzapros(const byte &telef)
{
 ...
 // ниже распечатаем все температуры   
    for (byte i = 0; i<size_arrayTemp; i++) 
    {
      TempName(DS18B20[i][8]); SIM800.print (Temper(DS18B20[i][8])); grad ();
    }
...
SIM800.print(F("BUS"));
  ...
  if (DTC[0] >0){
     ...
     if (DTC[i*4+1]<=0x0F) SIM800.print(F("0"));
     SIM800.print (DTC[i*4+1], HEX); 
     if (bitRead(DTC[i*4+2], 6)) SIM800.println (F(" Active")); 
