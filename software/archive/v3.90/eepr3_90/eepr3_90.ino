...
enum Heater_  { TTC_E, WBUS, HYDRONIC};                 
...
byte Heater         = TTC_E;       // в данном случае тип шины WBUS
...
bool CentrLock  = 0;              // управление ЦЗ вместо помпы и старта ДВС (1 - активно, 0 - неактивно)
byte PumpeTime  = 120;            // время работы помпы      (ЦЗ открыть), сек
